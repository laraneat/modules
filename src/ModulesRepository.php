...
class ModulesRepository implements Arrayable
{
     ...
     */
    public function getProviders(ModuleType $type = ModuleType::All): array
    {
        return collect($this->getModules($type))
     ...
     */
    public function getAliases(ModuleType $type = ModuleType::All): array
    {
     ...
     * @throws ModuleHasNonUniquePackageName
     */
    public function getModules(ModuleType $type = ModuleType::All): array
    {
        return match ($type) {
            ModuleType::All => $this->getAllModules(),
            ModuleType::App => $this->getAppModules(),
            ModuleType::Vendor => $this->getVendorModules(),
        };
    }
     ...
     */
    public function has(string $modulePackageName, ModuleType $type = ModuleType::All): bool
    {
        return array_key_exists($modulePackageName, $this->getModules($type));
    }

    /**
     * Get count from all modules.
     */
    public function count(ModuleType $type = ModuleType::All): int
    {
        return count($this->getModules($type));
    }

    /**
     * Find a specific module by its package name.
     */
    public function find(string $modulePackageName, ModuleType $type = ModuleType::All): ?Module
    {
        $modulePackageName = trim($modulePackageName);

        return $this->getModules($type)[$modulePackageName] ?? null;
    }
     ...
     */
    public function findOrFail(string $modulePackageName, ModuleType $type = ModuleType::All): Module
    {
        $module = $this->find($modulePackageName, $type);
     ...
     * @throws ModuleNotFound
     */
    public function delete(string $modulePackageName, ModuleType $type = ModuleType::All): bool
    {
        return $this->findOrFail($modulePackageName, $type)->delete();
    }
     ...
     * @return array<string, Module>
     */
    public function filterByName(string $moduleName, ModuleType $type = ModuleType::All): array
    {
        $moduleName = trim($moduleName);
        $modules = [];

        foreach($this->getModules($type) as $modulePackageName => $module) {
     ...
     * @throws ModuleNotFound
     */
    public function filterByNameOrFail(string $moduleName, ModuleType $type = ModuleType::All): array
    {
     ...
     * }>
     */
    public function toArray(ModuleType $type = ModuleType::All): array
    {
        return array_map(static fn (Module $module) => $module->toArray(), $this->getModules($type));
    }
